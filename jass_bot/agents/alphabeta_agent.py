...
class AlphaBetaAgent(AgentCheating):
    ...
    def _start_minimax(self, state: GameState, depth: int) -> (int, int):
        ...
        best_score = self._alphabeta(origin_node,
                                     depth_complete_tricks=depth,
        ...
        # assert best_score == highest_scoring_child.achievable_score, "Highest score returned from minimax is not the same as the highest soring child"
    ...
    # we always just return node.achievable_score,
    # so it's more in line with other alpha-beta pruning implementations
    def _alphabeta(self, node: GameTreeNode, depth_complete_tricks: int, alpha: float, beta: float, maximize: bool, first_call=False) -> float:
        if depth_complete_tricks == 0 or node.is_terminal():
            node.achievable_score = node.state.points[team[node.state.player]] * (1 if maximize else -1)
            return node.achievable_score
        ...
        best_child_score = sys.float_info.max * (-1 if maximize else 1)
        left_depth = -1
        for child in node.children.values():
            ...
            if maximize:
                ...
                alpha = max(alpha, best_child_score)
            else:
                if best_child_score < alpha:
                    break

                beta = min(beta, best_child_score)
