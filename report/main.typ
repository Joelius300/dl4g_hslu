...
Die verwendete Trumpfstrategie ist die Graf Heuristik @graf_jassen_nodate, die im Unterricht vorgestellt wurde. Sie erwies sich als die beste von allen evaluierten Ansätzen, darunter viele Variationen von Supervised Machine Learning. Mehr dazu in @supervised-trump.
...
Ein wichtiger Punkt ist die Auswertung dieser Strategien. Es ist klar, dass nicht die Metriken auf den Daten verwendet werden dürfen, da diese nur aussagen, wie gut die Modell die Daten imitiert. Wie aber bereits am Fall von AlphaZero @silver_mastering_2017 gezeigt, kann es gut sein, dass eine neuartike Taktike, die tiefe Korrelation mit menschlichem Spiel aufweist, besser ist als diejenigen, die lediglich Menschen imitieren. Aus diesem Grund wird jede Trumpfstrategie wie folgt ausgewertet: Es werden 128 Jass Partien auf jeweils 1000 Punkte gegen die Graf Heuristik gespielt und die Gewinnrate berechnet. Die Kartenstrategie ist für alle Spieler ISMCTS. So kann aufgezeigt werden, ob diese Trumpfstrategie im Spiel tatsächlich die Überhand hat, was der Fall ist, wenn sie eine Gewinnrate von über 50% erzielt.
...
Der einfachste Ansatz ist die alleinige Verwendung von den Swisslos Daten. Dies erzielte eine Gewinnrate von 40% gegen Graf. Ein spannenderer Ansatz ist ein Pre-Training Ansatz, wo zuerst die Graf Heuristik gelernt wird (mit einem generierten Datensatz) und anschliessend nur mit den besten Spielern von den Swisslos Daten ein Fine-Tuning gemacht wird.
...
Das Modell nimmt die One-Hot kodierten Handkarten (36x1 binär) + ob geschoben werden kann oder nicht (1x1 binär). Diese 37 Features werden in mehrere Linear Layers mit ReLU non-linearity gegeben. Bei jedem Layer werden die originalen Inputs hinzugefügt, um Skip-Connections zu ermöglichen. Am Ende ist ein Linear Layer, welcher die Logits für die 7 Trumpf Optionen (4 Farben + Une-ufe + Obe-abe + Schieben) ausgibt. Der verwendete Loss ist Cross-Entropy. Zusätzlich wird Dropout verwendet.
...
Zwei Payoff Funktionen werden verglichen. Binärer Sieger (1 für Gewinner, 0 für Verlierer) schlägt punktbasierte Payoffs (tatsächliche Punkte auf [0,1] skaliert) nicht nur in Anzahl gewonnen Spielen sondern auch in Partien auf 1000 Punkte.

Der C Parameter für UCB1 wird ebenfalls getuned. Am Ende wurde 8 als beste Option gewählt, was erstaunlich war, da bisher $sqrt(2)$ sehr gut zu performen schien, aber es zeigt sich, dass $1 < sqrt(2) < 5 < 10 < 7 < 8$. Diese Tests werden im finalen Setting, das heisst auf dem Deployment Server mit Root Parallelized ISMCTS mit 15 Workers und 9.75 Sekunden Zeitbudget getestet.

== Kurzer Abstecher in Reinforcement Learning

Mit den Unterlagen zu Reinforcement Learning gelang es leider nicht einen Bot mit RL zu trainieren. Es wurde jedoch versucht ein Gymnasium Environment @noauthor_gymnasium_nodate Wrapper aufbauend auf dem Jass-Kit GameSim zu erstellen, jedoch ohne Erfolg.

= Nennenswertes

Einige zusätzliche Dinge, die gemacht wurden und eventuell spannend sein könnte. Diese können im Code angeschaut werden.
